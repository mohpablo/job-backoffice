<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Job Applications" . (request()->has('archived') && request()->boolean('archived') ? ' (Archived)' : '')) }}
        </h2>
    </x-slot>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-100">
                    <td class="px-4 py-3 text-md font-medium ">
                        ...
                        <a href="{{ route('job-applications.show', $application->id) }}"> {{ $application->user->name }}</a>
                        @else
...
">
                    ...
                    <td class=" py-3">
                        <div class="flex items-center space-x-2">
                            @if (request()->has('archived') && request()->boolean('archived'))
                            <!-- Restore Button -->
                            <form action="{{ route('job-applications.restore', $application->id) }}" method="POST" class="inline">
                                @csrf
                                @method('PUT')
                                <button type="submit"
                            ...
                            <!-- Delete Button -->
                            <form action="{{ route('job-applications.destroy', $application->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
