<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Job categories" . (request()->has('archived') && request()->boolean('archived') ? ' (Archived)' : '')) }}
        </h2>
    </x-slot>
    ...
    <div class="overflow-x-auto p-6 ">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('job-categories.index') }}"
                ...
                <span class="font-medium">View Active Categories</span>
            </a>
            @else
            <!-- Archived -->
            <a href="{{ route('job-categories.index', ['archived' => true]) }}"
                ...
                <span class="font-medium">View Archived Categories</span>
            </a>
            @endif
        ...
        <table class="min-w-full divide-y divide-gray-200 rounded-lg shadow mt-4 bg-white">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-100">
                    <td class="px-6 py-2 text-md font-medium text-gray-900">
                        {{ $category->name }}
                    </td>
                    <td class=" py-2 whitespace-nowrap">
                        <div class="flex justify-start items-center space-x-4">
                            ...
                            <form action="{{ route('job-categories.restore', $category->id) }}" method="POST"
                                ...
                                @method('PUT')
                                <button type="submit"
                                    class="inline-flex items-center gap-1.5 text-gray-600 hover:text-gray-800 hover:bg-gray-50 px-3 py-2 rounded-md transition-colors duration-200">
                            ...
                            <a href="{{ route('job-categories.edit', $category->id) }}"
                                class="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-3 py-2 rounded-md transition-colors duration-200">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <form action="{{ route('job-categories.destroy', $category->id) }}" method="POST" class="inline">
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="inline-flex items-center gap-1.5 text-red-600 hover:text-red-800 hover:bg-red-50 px-3 py-2 rounded-md transition-colors duration-200">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <tr>
                    <td colspan="2" class="px-6 py-8 text-center">
                        <div class="text-gray-500">
                            ...
                            <p class="mt-2 text-sm font-medium">No job categories found.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
