@php
$user = auth()->user();
$formaction = $user->role === 'admin'
? route('companies.update', ['company' => $company->id,'redirecttolist' => request('redirecttolist')])
: route('my-company.update', ['id' => $company->id,]);
@endphp
        ...
        <div class="max-w-2xl   mx-auto p-6 bg-white shadow-md rounded">
            <form action="{{ $formaction }}" method="post">
                ...
                <div class="mb-4 p-6 bg-gray-50 border-gray-100 rounded-lg shadow-sm">
                    ...
                    <p class="text-sm mb-4 ">Edit the details of your company</p>
                    <!-- company name -->
                ...
                <div class="mb-4 p-6 bg-gray-50 border-gray-100 rounded-lg shadow-sm">
                    ...
                    <div class="mt-4">
                        <div>
                            ...
                            <x-text-input id="owner_name" class="block mt-1 w-full" type="text" name="owner_name" :value="old('owner_name', $company->owner->name)" autofocus autocomplete="username" />
                            <x-input-error :messages="$errors->get('owner_name')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="owner_email" :value="__('Owner Email read only')" />
                            <x-text-input disabled id="owner_email" class="block mt-1 w-full text-gray-400" type="email" name="owner_email" :value="old('owner_email',$company->owner->email)" autofocus autocomplete="username" />
                            <x-input-error :messages="$errors->get('owner_email')" class="mt-2" />
                        </div>

                        <!-- Password -->
                        <div class="mt-4">
                            <x-input-label for="owner_password" :value="__('Change owner password (optional , leave blank if not changed)')" />
                ...
                <div class="flex justify-end space-x-4 mt-2">
                    ...
                    <a href="{{ route('my-company.show', ['id' => $company->id]) }}"
                        class="px-4 py-2 rounded-md text-gray-500 hover:text-gray-700">
                        Cancel
                    </a>
                    @endif
